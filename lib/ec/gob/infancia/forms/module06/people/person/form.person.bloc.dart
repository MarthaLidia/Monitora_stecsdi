...
part of ec.gob.infancia.ecuadorsincero.forms.module06.person;
...
class FormPersonBloc extends BaseBloc<FormPersonBlocState> {
  ...
  onLoad() async {
    ...
    FormUtils.setQuestions(
      state: state,
      formId: formId,
      params: ['ps_01', Module06Constants.moduleId],
      code: code,
  ...
  _handleLoadForm() {
    ...
    sqliteDB.query(
      ...
      whereArgs: [formId, code, 'ps_01', Module06Constants.moduleId],
    ).then((answersDb) {
      ...
      sqliteDB.query(
        ...
        whereArgs: [formId, 'h_37', Module06Constants.moduleId],
      ).then((answersDb) {
        answersDb.map((item) => ModelFormAnswer.db(item)).toList().forEach(
          (item) {
            state.setFormAnswer(item.question, item);
