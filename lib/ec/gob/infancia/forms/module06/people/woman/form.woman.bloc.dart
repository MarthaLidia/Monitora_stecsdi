...
class FormWomanBloc extends BaseBloc<FormWomanBlocState> {
  ...
  onLoad() async {
    ...
    FormUtils.setQuestions(
      ...
      params: ['ms_01', 'ms_02', Module06Constants.moduleId],
      code: code,
      action: (questionId) {},
    );
  ...
  _handleLoadForm() {
    ...
    sqliteDB.query(
      ...
      whereArgs: [formId, code, 'ms_01', 'ms_02', Module06Constants.moduleId],
    ).then((answersDb) {
  ...
  bool _validateForm() {
    ...
    for (var question in auxq) {
      ...
      if (!state.formAnswers.containsKey(question.id)) {
        UtilsToast.showDanger("error 1 ${question.id}");
        state.setFormErrors(question.id, localizations.fieldIsRequiredMessage);
      } else if (state.formAnswers[question.id]!.other!.isEmpty) {
        UtilsToast.showDanger("error 2 ${question.id}");
        state.setFormErrors(question.id, localizations.fieldIsRequiredMessage);
      }
      state.questionsTextController['m_18_1']!.text;
      if (question.enabledBy.isNotEmpty) {
        ...
        if (isEnabled) {
          if (!state.formAnswers.containsKey(question.id)) {
            UtilsToast.showDanger("error 3 ${question.id}");
            state.setFormErrors(
                question.id, localizations.fieldIsRequiredMessage);
          } else if (state.formAnswers[question.id]!.other!.isEmpty) {
            UtilsToast.showDanger("error 4 ${question.id}");
            state.setFormErrors(
                question.id, localizations.fieldIsRequiredMessage);
          }
        } else {
    ...
    UtilsToast.showDanger(state.formErrors.values.toString());
