...
class FormPeopleBloc extends BaseBloc<FormPeopleBlocState> {
  ...
  onLoad() async {
    ...
    var formHeaders = await sqliteDB.query(
      ...
      whereArgs: [formId, Module03Constants.moduleId],
    );
    ...
    FormUtils.setQuestions(
      state: state,
      formId: formId,
      params: ['r', Module03Constants.moduleId],
      action: (questionId) {
        if (questionId == 'r_05') {
  ...
  _handleLoadForm() {
    sqliteDB.query(
      ...
      whereArgs: [formId, 'r', 0, Module03Constants.moduleId],
      ...
      sqliteDB.query(
        ...
        whereArgs: [formId, 'h_36', Module03Constants.moduleId],
      ).then((answersDb) {
      ...
      sqliteDB.query(
        ...
        whereArgs: [formId, 'h_10', 'h_11', 'h_12', Module03Constants.moduleId],
      ).then((answersDb) {
  ...
  _loadPeopleInfo() {
    ...
    sqliteDB.query(
      ...
      whereArgs: [formId, 'h_36', 0, Module03Constants.moduleId],
    ).then((answersDb) async {
      if (answersDb.isEmpty) {
        return;
      }
  ...
  handleSubmit() async {
    ...
    if (isValid) {
      ...
      UtilsHttp.checkConnectivity().then((_) {
        ...
        }, module: Module03Constants.moduleId);
      });
    } else {
  ...
  _handleRemovePerson(int code) async {
    if (totalPeople > 1) {
      ...
      await sqliteDB.delete(
        ...
        whereArgs: [formId, code, Module03Constants.moduleId],
      );
