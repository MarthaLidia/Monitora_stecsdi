...
class FormHouseBloc extends BaseBloc<FormHouseBlocState> {
  ...
  onLoad() async {
    ...
    FormUtils.setQuestions(
      state: state,
      formId: id,
      params: ['hs_01', Module03Constants.moduleId],
      action: (questionId) {
        if (questionId == 'h_06' || questionId == 'h_07') {
          ...
          sqliteDB.update(
            ...
            whereArgs: [formHeader.id, Module03Constants.moduleId],
          );
        }
        if (questionId == 'h_15' &&
  ...
  _handleNewForm() async {
    ...
    sqliteDB.transaction((txn) async {
      formHeader = ModelFormHeader(
        ...
        moduleName: Module03Constants.moduleName,
  ...
  _handleEditForm() {
    ...
    sqliteDB.query(
      ...
      var answersDb = await sqliteDB.query(
        ...
        whereArgs: [formHeader.id, 'hs_01', code, Module03Constants.moduleId],
      );
  ...
  _handleOldLogic({
    ...
    if (formHeader.tryNumber == 3 || doesntAllowTries) {
      ...
      sqliteDB.delete(
        ...
        whereArgs: [formId, question, code, Module03Constants.moduleId],
      );
