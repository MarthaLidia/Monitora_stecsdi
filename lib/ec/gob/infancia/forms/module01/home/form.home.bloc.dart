...
class FormHomeBloc extends BaseBloc<FormHomeBlocState> {
  ...
  onLoad() async {
    ...
    FormUtils.setQuestions(
      ...
      action: _handleFormInputChange,
    );

    sqliteDB.query('AnswerValidation').then((value) {
      var validations = value
          .map((data) => ModelAnswerValidation.db(data))
          .toList()
          .cast<ModelAnswerValidation>();
      state.addData('formValidations', validations);
    });

    _handleLoadForm();
  }

  _handleFormInputChange(questionId) {
    ...
    _handleRestrictionException(questionId);
  }
  ...
  _handleQuestionChange(
    ...
    switch (question) {
      ...
      case 'h_54':
        answerComparator = '2';
        nextQuestion = ['h_55', 'h_56', 'h_57', 'h_58', 'h_59'];
        break;
      case 'h_56':
        answerComparator = '5';
        nextQuestion = ['h_57'];
    ...
    _handleRestrictionException(question);
  }
    ...
    for (var question in state.questions) {
      ...
      switch (question.id) {
        ...
        case 'h_50':
          ...
          break;
        case 'h_54':
          conditions['1'] = ['h_57', 'h_58', 'h_59'];
          conditions['2'] = ['h_55', 'h_56', 'h_57', 'h_58', 'h_59'];
          conditions['3'] = ['h_55', 'h_56', 'h_57', 'h_59'];
          break;
        case 'h_56':
          conditions['5'] = ['h_57'];
          break;
        ...
        if (!state.formAnswers.containsKey(question.id)) {
          ...
          if (state.formAnswers[question.id]!.other!.isEmpty) {
            ...
            for (var key in conditions.keys) {
              if (state.formAnswers[question.id]!.other == key) {
                for (var id in (conditions[key] ?? [])) {
                  if (!state.formAnswers.containsKey(id)) {
                    state.setFormErrors(
                        id, localizations.fieldIsRequiredMessage);
  ...
  _validateSpecificFields() {
    for (var question in ['h_19', 'h_21', 'h_30', 'h_32', 'h_57', 'h_59']) {
  ...
  _handleRestrictionException(String question) {
    var validations = state.formValidations
        .where((item) => item.restrictionQuestion == question);

    if (validations.isNotEmpty) {
      Map<String, bool> cond = {};
      for (var val in validations) {
        if (val.conditionQuestion == question ||
            (cond.containsKey(question) && cond[question]!)) {
          continue;
        }

        if (state.formAnswers.containsKey(val.restrictionQuestion)) {
          var restricted = state.formAnswers[val.restrictionQuestion];
          if (restricted!.complete) {
            state.setFormErrors(val.restrictionQuestion, null);
            continue;
          }
        }
        ...
        if (state.formAnswers.containsKey(val.restrictionQuestion)) {
          if (question != 'h_28') {
            cond[val.restrictionQuestion] =
                (state.formAnswers[val.conditionQuestion]?.other ?? '-1') ==
                        val.conditionAnswer &&
                    (state.formAnswers[val.restrictionQuestion]?.other ??
                            '-1') ==
                        val.restrictionAnswer;
            if (cond[val.restrictionQuestion] ?? false) {
              state.setFormErrors(val.restrictionQuestion, val.message);
              continue;
            }
          } else {
            ...
            if (h28.isNotEmpty) {
              try {
                ...
                if (value < 0 || value > 25) {
                  ...
                  var ans = val.conditionAnswer == '3'
                      ? '3'
                      : val.conditionAnswer == '4'
                          ? '4'
                          : '-1';

                  var firstCondition = val.conditionAnswer == ans &&
                      (state.formAnswers[val.conditionQuestion]?.other ??
                              '0') ==
                          ans;
                  var secondCondition = state.formAnswers
                          .containsKey(val.conditionQuestion) &&
                      state.formAnswers.containsKey(val.restrictionQuestion);
                  var thirdCondition = value > 1;
                  cond[val.restrictionQuestion] =
    ...
    if (question == 'h_45') {
      var selectedQty =
          (state.formAnswers['h_45']?.other ?? '').split('|').length - 1;
      if (selectedQty > 2) {
        state.setFormErrors('h_45', localizations.fieldFormError_FOOD_MATCH);
      }
    }
  }
}
