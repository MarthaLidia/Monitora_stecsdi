...
part of ec.gob.infancia.ecuadorsincero.forms.module01.woman;
...
class FormWomanBloc extends BaseBloc<FormWomanBlocState> {
  ...
  onLoad() async {
    ...
    FormUtils.setQuestions(
      ...
      params: ['ms_01', 'ms_02', Module01Constants.moduleId],
      code: code,
      action: (questionId) {},
    );
  ...
  _handleLoadForm() {
    ...
    sqliteDB.query(
      ...
      whereArgs: [formId, code, 'ms_01', 'ms_02', Module01Constants.moduleId],
    ).then((answersDb) {
  ...
  _handleRestrictionException(String question) {
    if (question == 'm_14' || question == 'm_15') {
      if (state.formAnswers.containsKey('m_14') &&
          state.formAnswers.containsKey('m_15')) {
        var qty = int.parse(state.formAnswers['m_14']?.other ?? '0');
        if (qty == 6) {
          qty = 5;
        }
        var selectedQty =
            (state.formAnswers['m_15']?.other ?? '').split('|').length - 1;
        if (qty != selectedQty) {
          state.setFormErrors('m_15', localizations.fieldFormError_FOOD_MATCH);
        }
      }
    }
  }
