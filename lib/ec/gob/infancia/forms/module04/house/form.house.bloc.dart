...
class FormHouseBloc extends BaseBloc<FormHouseBlocState> {
  ...
  _handleQuestionChange(
    ...
    print(formAnswer);
    state.addData('showContinuar', true);
    if(formAnswer.answer==4507){
      state.addData('tipoControl', FormEmbarazoWidget.routeName);
    }else if(formAnswer.answer==4515){
      state.addData('tipoControl', '');
    }
    var questionsToClean = [];
    if (formAnswer.question == 'h_07') {
      questionsToClean = [
        'p_05',
        'p_06',
        'p_04',
  ...
  handleSubmit() {
    ...
    Navigator.of(context)
        .pushNamed(state.data["tipoControl"])
        .then((value) {
  ...
  _handleTabBtnNroDoc(String cedula){
    ...
    getInfoPersona(cedula ?? "")
      ...
      if(value!=null){
        ...
        if(dates.isNotEmpty){
          var split1=dates.split("T");
          var split2=split1[0].split("-");
          ndate=split2[2]+"/"+split2[1]+"/"+split2[0];
        }
        state.questionsTextController["p_08"]?.text=ndate;
      }
      print(value);
    })
        .catchError((onError){
